...
<body>
	...
	<div class="page-container">
		...
		<main class="main-content">
			...
			<div class="container">
				<form action="resumeModify" name="resumeForm" method="post"
					onsubmit="return validateForm()" enctype="multipart/form-data">
					<input type="hidden" name="member_idx"
						value="${member.member_idx }"> <input type="hidden"
						name="resume_idx" value="${resume.resume_idx}"> <input
						type="hidden" name="exp_idx" value="${exp.exp_idx}"> <input
					...
					<div class="resume-form">
						<div class="section">
							...
							<div class="profile-section">
								<div class="avatar-container">
									...
									<!-- 실제 업로드 input (숨김) -->
									<input type="file" name="resume_img" id="file1"
										accept="image/*" style="display: none;">

									<!-- 커스텀 버튼 -->
									<button type="button" class="upload-btn"
										onclick="document.getElementById('file1').click();">업로드</button>
								</div>
						...
						<div class="section">
							...
							<div class="form-row">
								<div class="form-group">
									<label>학위</label> <select name="degree">
										<option value="" selected disabled>-- 선택 --</option>
										<c:forEach var="d" items="${degreeList}">
											<option value="${d.code}"
												${resume.degree == d.code ? 'selected' : ''}>
												${d.code_name}</option>
										</c:forEach>
									</select>
								</div>
								<div class="form-group">
									<label>상태</label> <select name="degreetype">
										<option value="" selected disabled>-- 선택 --</option>
						...
						<div class="section">
									...
									<div class="date-input-container">
										<select name="experience">
											...
											<c:forEach var="e" items="${experienceList}">
												<option value="${e.code}"
													${exp.experience == e.code ? 'selected' : ''}>
													${e.code_name}</option>
											</c:forEach>
										</select>
									</div>
									...
									<div class="date-input-container">
										<select name="occupation">
											...
											<c:forEach var="o" items="${occupationList}">
												<option value="${o.code}"
													${exp.occupation == o.code ? 'selected' : ''}>
													${o.code_name}</option>
											</c:forEach>
										</select>
									</div>
							...
							<div class="form-group">
								...
								<textarea name="responsibilities"
									placeholder="주요 담당 업무 및 성과를 입력하세요" autocomplete="off"></textarea>
							</div>
						</div>
...
<script type="text/javascript">
	...
	gradeSelect.addEventListener("change", function() {
		...
		if (gradeInput.value && !isNaN(parseFloat(gradeInput.value))
				&& parseFloat(gradeInput.value) > maxGrade) {
	...
	gradeInput.addEventListener("input", function() {
		...
		if (this.value && !isNaN(parseFloat(this.value))
				&& parseFloat(this.value) > maxGrade) {
			this.value = maxGrade;
		}
	});
