...
import com.itwillbs.illusion.service.ilMailService;
import com.itwillbs.illusion.service.ilMemberService;
import com.itwillbs.illusion.vo.ilMailAuthInfo;
import com.itwillbs.illusion.vo.ilMemberVO;

@RestController // JSON 응답을 위한 어노테이션
@RequestMapping("/member")
public class ilMemberController {

	@Autowired
	private ilMemberService memberService;

	@Autowired
	private ilMailService mailService;

	/**
	 * 이메일 인증번호 발송 요청 API 클라이언트에서 이메일을 JSON으로 보내면 해당 이메일로 인증번호를 생성해 메일 전송 후 DB 저장
	 */
	@PostMapping("email-auth")
	public Map<String, Object> sendAuthMail(@RequestBody ilMemberVO member) {
		// 인증 메일을 발송하고 인증 정보 반환
		System.out.println("안되나?");
		ilMailAuthInfo mailAuthInfo = mailService.sendAuthMail(member);
		// DB에 인증정보 등록
	...
	public Map<String, Object> checkAuthMail(@RequestBody ilMailAuthInfo mailAuthInfo) {
		// 인증번호 검사 처리
